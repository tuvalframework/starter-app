...
shell.exec(`docker cp ${container_name}:/server/portal/static/appstore.json  ../../docker/appstore.json`)


shell.echo('App file creating...');
tuval.appPackager('./dist/index.js', `../../docker/applications/${appName}.app`)

shell.echo('App store info updating...');
const path = require('path');
const fs = require('fs');
const a = fs.readFileSync('../../docker/appstore.json', 'utf8');
...
fs.writeFileSync('../../docker/appstore.json', aa, 'utf8');
shell.echo('App info done.');

shell.echo('Exposing appstore info to realm container');
shell.exec(`docker cp  ../../docker/appstore.json  ${container_name}:/server/portal/static/appstore.json`)
shell.echo(`Exposing ${appName}.app to realm container appstore`);
shell.exec(`docker cp  ../../docker/applications/${appName}.app  ${container_name}:/server/portal/static/applications/${appName}.app`)
shell.echo(`All done.`);